...
class Question:
    ...
    def get_prompt_for_mcp(self):
        return self._get_prompt(include_choices=True)

    def get_prompt_for_cp(self):
...
class QuestionWithExemplars(Question):
    ...
    def get_prompt_for_mcp(self):
        prompt = super().get_prompt_for_mcp()
        if len(self.exemplars):
            exemplar_prompts = [e.get_prompt_for_mcp() for e in self.exemplars]
    ...
    def get_prompt_for_cp(self):
        prompt = super().get_prompt_for_cp()
        if len(self.exemplars):
            exemplar_prompts = [e.get_prompt_for_cp() for e in self.exemplars]
            exemplars = "\n\n".join(exemplar_prompts)
            return f"{exemplars}\n\n{prompt}"
        else:
            return prompt
